...
var ThreadStream = class {
	...
	*/
	...
	#onErrorListeners = /* @__PURE__ */ new Set();
	#messagesIterable;
	#valuesProjection;
	#toolCallsIterable;
	#subgraphsIterable;
	...
	*/
	...
	#fireOnError(error) {
		for (const listener of this.#onErrorListeners) try {
			listener(error);
		} catch {}
	}
	async close() {
	...
	*/
	...
	#handleIncoming(message) {
		...
		if (!pending) {
			if (message.type === "error" && message.id == null) this.#fireOnError(new require_error.ProtocolError(message));
			return;
		}
