...
function readRunId(data) {
	const record = data;
	return typeof record.run_id === "string" ? record.run_id : void 0;
}
function readUsage(data) {
	const usage = data.usage;
	return usage != null && typeof usage === "object" && !Array.isArray(usage) ? usage : void 0;
}
function readFinishMetadata(data) {
	const record = data;
...
var StreamingMessage = class {
	...
	async #assembleMessage() {
		...
		let metadata = { ...this.assembled.finishMetadata ?? {} };
		let finishReason = this.assembled.finishReason;
		for await (const event of this.#events) switch (event.event) {
			...
			case "message-finish": {
				...
				if (finishMetadata != null) metadata = {
					...metadata,
					...finishMetadata
				};
				break;
		...
		return new AIMessage({
			...
			additional_kwargs: {
				...
				...this.assembled.runId != null ? { run_id: this.assembled.runId } : {},
				...this.metadata != null ? { metadata: this.metadata } : {},
				...this.assembled.usage != null ? { usage: this.assembled.usage } : {}
			},
			response_metadata: {
...
var MessageAssembler = class {
	...
	*/
	consume(event) {
		...
		const runKey = runId != null ? `${namespaceNodeKey}::${runId}` : void 0;
		const fallbackKey = this.activeByNamespaceNode.get(namespaceNodeKey);
		const fallbackMessage = fallbackKey != null ? this.activeMessages.get(fallbackKey) : void 0;
		const activeKey = (runKey != null && this.activeMessages.has(runKey) ? runKey : void 0) ?? (runId == null || fallbackMessage?.runId == null ? fallbackKey : void 0);
		if (!activeKey) {
	...
	clearActiveMessage(activeKey, namespaceNodeKey) {
		this.activeMessages.delete(activeKey);
		if (this.activeByNamespaceNode.get(namespaceNodeKey) === activeKey) this.activeByNamespaceNode.delete(namespaceNodeKey);
	}
