...
const PARKED_INTERRUPT_SETTLE_MS = 5e3;
...
var StreamController = class {
	...
	*/
	#interruptReplayThroughSeq;
	/** Unknown inputs waiting to be classified as live or replayed history. */
	#pendingHydrationInterruptEvents = [];
	#settleParkedInterruptsPromise;
	/**
	...
	*/
	constructor(options) {
		...
		this.queueStore = new require_store.StreamStore(require_queue_adapter.EMPTY_QUEUE);
		this.#submitter = new require_submit_coordinator.SubmitCoordinator({
			...
			onSubmitStart: () => {
				...
				this.#interruptCommandPending = this.#hydratedActiveInterruptIds != null;
	...
	*/
	async hydrate(threadId) {
		...
		if (changed) {
			...
			*/
			this.queueStore.setState(() => require_queue_adapter.EMPTY_QUEUE);
			await teardown;
		}
		...
		try {
			...
			*/
			if (Array.isArray(state?.tasks)) {
				...
				const { activeInterrupts, activeIds } = collectActiveInterruptsFromTasks(state.tasks);
				hasActiveInterrupts = activeInterrupts.length > 0;
				for (const id of activeIds) this.#resolvedInterrupts.delete(id);
				this.rootStore.setState((s) => ({
					...s,
				...
				if (this.#submitGeneration === generationAtFetch) {
					...
					this.#interruptReplayThroughSeq = void 0;
				}
			}
		} catch (error) {
			if (error?.status !== 404) {
		...
		* should be eager.
		*/
		this.#submitter.hydrateQueue(hydratedThreadId);
		/**
		...
		*/
		if (threadExists && threadActive) {
			...
			*/
			if (threadActive && !hasActiveInterrupts) this.rootStore.setState((s) => s.isLoading ? s : {
				...s,
				isLoading: true
			});
		}
	...
	*/
	async #teardownThread() {
		...
		*/
		...
		this.#threadErrorUnsubscribe = void 0;
		/**
 ...
 /**
				...
				*/
				while (!this.#disposed && pumpGeneration === this.#rootPumpGeneration) {
					...
					if (!subscription.isPaused) {
						...
						if (snapshot.isLoading && snapshot.error == null) this.rootStore.setState((s) => ({
							...s,
							error: /* @__PURE__ */ new Error("Thread event stream closed before the active run completed."),
							isLoading: false
						}));
						break;
					}
	...
	*/
	#recordInterrupt(event) {
		...
		if (typeof interruptId !== "string") return;
		const eventSeq = typeof event.seq === "number" ? event.seq : void 0;
		if (this.#resolvedInterrupts.has(interruptId)) {
			if (!(this.#hydratedActiveInterruptIds == null || typeof this.#interruptReplayThroughSeq === "number" && eventSeq != null && eventSeq > this.#interruptReplayThroughSeq)) return;
			this.#resolvedInterrupts.delete(interruptId);
		}
		const isUnknownInterrupt = this.#hydratedActiveInterruptIds != null && !this.#hydratedActiveInterruptIds.has(interruptId);
		...
		if (isUnknownInterrupt && barrier != null && eventSeq != null && eventSeq <= barrier) return;
	...
	*/
	#advanceInterruptReplayBarrier() {
		if (this.#hydratedActiveInterruptIds == null) {
			this.#discardPendingInterruptEvents();
			return;
		}
		const throughSeq = this.#thread?.ordering?.lastAppliedThroughSeq;
		if (typeof throughSeq === "number") this.#interruptReplayThroughSeq = throughSeq;
		else {
			this.#hydratedActiveInterruptIds = null;
			this.#interruptReplayThroughSeq = void 0;
		}
		...
		for (const event of pendingEvents) this.#recordInterrupt(event);
	}
	#discardPendingInterruptEvents() {
		this.#interruptCommandPending = false;
		this.#pendingInterruptEvents = [];
	}
	/**
	...
	*/
	async #reconcilePendingInterruptsFromServer() {
		const threadId = this.#currentThreadId;
		if (threadId == null || this.#disposed) return null;
		const generationAtFetch = this.#submitGeneration;
		try {
			const state = await this.#fetchHydrationState();
			if (this.#disposed || this.#currentThreadId !== threadId || this.#submitGeneration !== generationAtFetch) return null;
			if (!Array.isArray(state?.tasks)) return null;
			const { activeInterrupts, activeIds } = collectActiveInterruptsFromTasks(state.tasks);
			const pendingInterrupts = activeInterrupts.filter((interrupt) => interrupt.id == null || !this.#resolvedInterrupts.has(interrupt.id));
			this.rootStore.setState((s) => ({
				...s,
	...
	#settleParkedInterruptsOnTerminal(event) {
		if (this.#pendingHydrationInterruptEvents.length === 0) return;
		if (event.method !== "lifecycle") return;
		if (!require_namespace.isRootNamespace(event.params.namespace)) return;
		const status = event.params.data?.event;
		if (status !== "interrupted" && status !== "completed" && status !== "failed") return;
		this.#settleParkedInterrupts();
	}
	/**
	...
	async #settleParkedInterrupts() {
		const makePromise = async () => {
			try {
				...
				for (;;) {
					if (this.#disposed || this.#currentThreadId !== threadId || this.#submitGeneration !== generation || this.#pendingHydrationInterruptEvents.length === 0) return;
					const parkedIds = new Set(this.#pendingHydrationInterruptEvents.map((event) => event.params.data.interrupt_id));
					const state = await this.#reconcilePendingInterruptsFromServer();
					if (state != null) {
						if ([...this.#hydratedActiveInterruptIds ?? []].some((id) => parkedIds.has(id)) || !isThreadStateActive(state)) {
							this.#pendingHydrationInterruptEvents = [];
							return;
						}
					}
				...
				this.#settleParkedInterruptsPromise = void 0;
			}
		};
	...
	*/
	async #awaitResumedRunTerminal(signal) {
		const terminal = await this.#awaitRootTerminal(signal, { skipInterruptedUntilRunning: true });
		if (!signal.aborted && !this.#disposed && (terminal.event === "interrupted" || terminal.event === "completed")) await this.#reconcilePendingInterruptsFromServer();
		return terminal;
	}
	#awaitRootTerminal(signal, options) {
		return new Promise((resolve) => {
			...
			const unsubscribeError = this.#thread?.onError((error) => finish({
				event: "failed",
				error: error.message
			}));
...
* id dedupe.
*/
function collectActiveInterruptsFromTasks(tasks) {
