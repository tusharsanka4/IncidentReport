...
function assembledToBaseMessage(input) {
	const { id, role, blocks, namespace, node, runId, toolCallId, metadata, usage, finishMetadata, finishReason } = input;
	...
	switch (role) {
		case "human": return new _langchain_core_messages.HumanMessage({
			...
			...Object.keys(responseMetadata).length > 0 ? { response_metadata: responseMetadata } : {}
		});
