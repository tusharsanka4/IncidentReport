...
function shouldPreferValuesMessage(valuesMessage, streamedMessage, options) {
...
*/
function valuesAddsMetadataEnrichment(valuesMessage, streamedMessage, options) {
...
*/
function retainsAllNestedKeys(candidate, baseline) {
	if (baseline == null || typeof baseline !== "object") return true;
	if (candidate == null || typeof candidate !== "object" || Array.isArray(baseline) !== Array.isArray(candidate)) return false;
	if (Array.isArray(baseline)) {
		const candidateArr = candidate;
		const baselineArr = baseline;
		if (candidateArr.length < baselineArr.length) return false;
		for (let i = 0; i < baselineArr.length; i += 1) if (!retainsAllNestedKeys(candidateArr[i], baselineArr[i])) return false;
		return true;
	}
	const candidateRecord = candidate;
	const baselineRecord = baseline;
	for (const key of Object.keys(baselineRecord)) {
		if (!Object.prototype.hasOwnProperty.call(candidateRecord, key)) return false;
		if (!retainsAllNestedKeys(candidateRecord[key], baselineRecord[key])) return false;
	}
	return true;
}
/**
...
//#endregion
export { buildMessageIndex, messagesEqual, reconcileMessagesFromValues, shouldPreferValuesMessage };
