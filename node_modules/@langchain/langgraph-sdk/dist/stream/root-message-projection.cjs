	...
	*/
	applyValues(nextValues, nextMessages, opts) {
		...
		const reconciliation = require_message_reconciliation.reconcileMessagesFromValues({
			...
			preferValuesMessage: (valuesMessage, streamedMessage) => require_message_reconciliation.shouldPreferValuesMessage(valuesMessage, streamedMessage, { allowInPlaceMutations: !addOnly }),
			addOnly
		});
