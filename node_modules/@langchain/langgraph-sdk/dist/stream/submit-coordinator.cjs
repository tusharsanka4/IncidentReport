...
require("./queue-adapter.cjs");
const require_queue_adapter_agent_server = require("./queue-adapter-agent-server.cjs");
const require_queue_adapter_local = require("./queue-adapter-local.cjs");
...
function assertServerQueueCapability(value) {
	if (!(typeof value === "object" && value != null && typeof value.create === "function" && typeof value.list === "function" && typeof value.cancel === "function")) throw new Error("queue: \"server\" requires client.runs to implement create/list/cancel");
}
/**
...
var SubmitCoordinator = class {
	...
	*/
	...
	constructor(params) {
		...
		const serverQueueEnabled = typeof params.options.transport !== "object" && params.options.queue === "server";
		if (serverQueueEnabled) assertServerQueueCapability(params.options.client.runs);
		this.#queueAdapter = serverQueueEnabled ? new require_queue_adapter_agent_server.AgentServerQueueAdapter(params.options.client.runs, params.options.assistantId, params.queueStore, onQueueError, (threadId) => params.ensureThread(threadId, true)) : new require_queue_adapter_local.LocalQueueAdapter(params.queueStore, onQueueError);
	...
	*/
	async submit(input, options) {
		...
		const pendingServerCreate = wasSelfCreated || this.#isSelfCreatedThreadId(currentThreadId);
		...
		const hasActiveRun = !wasSelfCreated && (this.#runAbort != null && !this.#runAbort.signal.aborted || this.#rootStore.getSnapshot().isLoading);
		...
		if (hasActiveRun && strategy === "enqueue") {
			try {
				await this.#queueAdapter.enqueue(currentThreadId, input ?? void 0, options);
			} catch (error) {
				try {
		...
		try {
			...
			const boundConfig = require_dispatch_config.bindThreadConfig(options?.config, currentThreadId);
			...
			commandPromise.then(() => {
				...
				if (pendingServerCreate) this.#abandonDeferredRootPump();
			});
	...
	*/
	#scheduleQueueDrain() {
		setTimeout(() => {
			...
			this.#queueAdapter.onIdle?.((values, drainOptions) => this.submit(values, drainOptions).catch(() => {}));
		}, 0);
	}
	/**
	...
	*/
	scheduleQueueDrainOnObservedIdle() {
		this.#scheduleQueueDrain();
	}
	/**
