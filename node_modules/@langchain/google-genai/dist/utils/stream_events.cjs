...
async function* convertGoogleGenAIStream(source, options = {}) {
	...
	for await (const response of source) {
		...
		if (shouldStreamUsage && "usageMetadata" in response && response.usageMetadata) {
			usageSnapshot = require_common.convertUsageMetadata(response.usageMetadata, options.model ?? "");
			yield {
				event: "usage",
				usage: usageSnapshot
			};
		...
		if (!parts) continue;
		let toolIdx = 0;
		for (const part of parts) if ("text" in part && part.text) {
...
//#endregion
exports.convertGoogleGenAIStream = convertGoogleGenAIStream;
