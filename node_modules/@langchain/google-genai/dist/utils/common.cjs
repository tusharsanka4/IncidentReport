...
function mapGenerateContentResultToChatResult(response, extra) {
	if (!response.candidates || response.candidates.length === 0 || !response.candidates[0]) throw new require_errors.EmptyContentError({ blockReason: response.promptFeedback?.blockReason });
	const [candidate] = response.candidates;
	const { content: candidateContent, ...generationInfo } = candidate;
	if (!candidateContent) throw new require_errors.EmptyContentError({ finishReason: generationInfo.finishReason });
