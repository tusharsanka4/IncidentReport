...
function mapGenerateContentResultToChatResult(response, extra) {
	if (!response.candidates || response.candidates.length === 0 || !response.candidates[0]) throw new EmptyContentError({ blockReason: response.promptFeedback?.blockReason });
	const [candidate] = response.candidates;
	const { content: candidateContent, ...generationInfo } = candidate;
	if (!candidateContent) throw new EmptyContentError({ finishReason: generationInfo.finishReason });
...
//#endregion
export { _FUNCTION_CALL_THOUGHT_SIGNATURES_MAP_KEY, convertAuthorToRole, convertBaseMessagesToContent, convertMessageContentToParts, convertResponseContentToChatGenerationChunk, convertToGenerativeAITools, convertUsageMetadata, getMessageAuthor, mapGenerateContentResultToChatResult };
