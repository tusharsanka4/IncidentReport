	...
	*/
	async tick(options = {}) {
		...
		for await (const { task, error, signalAborted } of taskStream) {
			...
			if (require_errors.isGraphInterrupt(error)) graphBubbleUp = require_errors.isGraphInterrupt(graphBubbleUp) ? new require_errors.GraphInterrupt([...graphBubbleUp.interrupts, ...error.interrupts]) : error;
