...
import { channelsFromCheckpoint, createCheckpoint, deltaChannelsToSnapshot, exitDeltaTaskId, isDeltaChannel } from "../channels/base.js";
...
var PregelLoop = class PregelLoop {
	...
	*/
	...
	constructor(params) {
		...
		this.lifecycleEvents = params.hasGraphCallbacks ? [] : void 0;
	...
	async tick(params) {
		...
		if (![INPUT_DONE, INPUT_RESUMING].includes(this.input)) await this._first(inputKeys);
			...
			for (const [ch, v] of writes) {
				...
				if (channel != null && isDeltaChannel(channel) && _isOverwriteValue(v)) this._deltaChannelsWithOverwrite.add(ch);
			}
	...
	*/
	...
	async _first(inputKeys) {
		...
		if (cachedIsResuming) this.lifecycleEvents?.push({
			runId: this.manager?.runId,
			status: this.status,
			checkpointId: this.checkpoint.id,
			checkpointNs: [...this.checkpointNamespace]
