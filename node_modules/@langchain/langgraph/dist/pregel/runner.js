import { CONFIG_KEY_ABORT_SIGNALS, CONFIG_KEY_CALL, CONFIG_KEY_SCRATCHPAD, ERROR, ERROR_SOURCE_NODE, INTERRUPT, NO_WRITES, RESUME, RETURN } from "../constants.js";
import { GraphInterrupt, isGraphBubbleUp, isGraphDrained, isGraphInterrupt } from "../errors.js";
	...
	*/
	async tick(options = {}) {
		...
		for await (const { task, error, signalAborted } of taskStream) {
			...
			if (isGraphInterrupt(error)) graphBubbleUp = isGraphInterrupt(graphBubbleUp) ? new GraphInterrupt([...graphBubbleUp.interrupts, ...error.interrupts]) : error;
