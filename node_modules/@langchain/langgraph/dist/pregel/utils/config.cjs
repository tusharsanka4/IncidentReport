...
function mergeCallbacks(base, provided) {
	...
	const dedupeHandlers = (handlers) => handlers.filter((handler, index) => handlers.findIndex((candidate) => candidate === handler || candidate.name === "StreamMessagesHandler" && handler.name === "StreamMessagesHandler") === index);
	return new _langchain_core_callbacks_manager.CallbackManager(provided._parentRunId, {
		...
		inheritableHandlers: dedupeHandlers(base.inheritableHandlers.concat(provided.inheritableHandlers)),
