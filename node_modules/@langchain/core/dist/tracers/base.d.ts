...
declare abstract class BaseTracer extends BaseCallbackHandler {
   ...
   * when callbacks are backgrounded, so we expose it as a separate method here.
   */
  _createRunForToolStart(tool: Serialized, input: string | Record<string, unknown>, runId: string, parentRunId?: string, tags?: string[], metadata?: KVMap, name?: string): {
    name: string;
    run_type: string;
    extra?: KVMap;
    error?: string;
  ...
  handleToolStart(tool: Serialized, input: string | Record<string, unknown>, runId: string, parentRunId?: string, tags?: string[], metadata?: KVMap, name?: string, _toolCallId?: string): Promise<Run>;
