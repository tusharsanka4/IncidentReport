		...
		async function* wrapInputForTracing() {
			for await (const chunk of inputGenerator) {
				if (finalInputSupported) {
					if (finalInput === void 0) finalInput = chunk;
					else try {
	...
	*/
	...
	async *_streamEventsV2(input, options, streamOptions) {
		...
		async function consumeRunnableStream() {
			...
			try {
				if (config.signal) {
					if ("any" in AbortSignal) signal = AbortSignal.any([abortController.signal, config.signal]);
					else {
	...
	async invoke(input, options) {
		...
		try {
			...
			for (let i = 0; i < initialSteps.length; i += 1) {
				const promise = initialSteps[i].invoke(nextStepInput, patchConfig(config, { callbacks: runManager?.getChild(this.omitSequenceTags ? void 0 : `seq:step:${i + 1}`) }));
	...
	async batch(inputs, options, batchOptions) {
		...
		try {
			for (let i = 0; i < this.steps.length; i += 1) {
				const promise = this.steps[i].batch(nextStepInputs, runManagers.map((runManager, j) => {
...
//#endregion
export { Runnable, RunnableAssign, RunnableBinding, RunnableEach, RunnableLambda, RunnableMap, RunnableParallel, RunnablePick, RunnableRetry, RunnableSequence, RunnableToolLike, RunnableTraceable, RunnableWithFallbacks, _coerceToDict, _coerceToRunnable, convertRunnableToTool };
