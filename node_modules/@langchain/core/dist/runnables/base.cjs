...
var Runnable = class extends require_load_serializable.Serializable {
	...
	*/
	...
	async _callWithConfig(func, input, options) {
		...
		try {
			...
			output = await require_signal.raceWithSignal(promise, config.signal);
		} catch (e) {
	...
	*/
	...
	async *_streamEventsV2(input, options, streamOptions) {
		...
		const config = require_config.ensureConfig(options);
		const runId = config.runId ?? require_utils_uuid_index.v7();
		config.runId = runId;
	...
	async invoke(input, options) {
		...
		try {
			...
			for (let i = 0; i < initialSteps.length; i += 1) {
				const promise = initialSteps[i].invoke(nextStepInput, require_config.patchConfig(config, { callbacks: runManager?.getChild(this.omitSequenceTags ? void 0 : `seq:step:${i + 1}`) }));
				nextStepInput = await require_signal.raceWithSignal(promise, config.signal);
			}
