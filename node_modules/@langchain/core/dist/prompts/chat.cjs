...
var ChatPromptTemplate = class ChatPromptTemplate extends BaseChatPromptTemplate {
	...
	async _parseImagePrompts(message, inputValues) {
		...
		message.content = await Promise.all(message.content.map(async (item) => {
			...
			const formattedUrl = await require_prompt.PromptTemplate.fromTemplate(imageUrl, { templateFormat: this.templateFormat }).format(inputValues);
	...
	async formatMessages(values) {
		...
		for (const promptMessage of this.promptMessages) if (promptMessage instanceof require_base.BaseMessage) resultMessages.push(await this._parseImagePrompts(promptMessage, allValues));
			...
			if (this.templateFormat === "mustache") inputValues = { ...allValues };
				...
				if (!(inputVariable in allValues) && !(isMessagesPlaceholder(promptMessage) && promptMessage.optional)) throw require_errors_index.addLangChainErrorFields(/* @__PURE__ */ new Error(`Missing value for input variable \`${inputVariable.toString()}\``), "INVALID_PROMPT_INPUT");
	...
	static fromTemplate(template, options) {
		const humanTemplate = new HumanMessagePromptTemplate({ prompt: require_prompt.PromptTemplate.fromTemplate(template, options) });
