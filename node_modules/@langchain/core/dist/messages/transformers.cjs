...
async function _trimMessagesHelper(messages, options) {
	...
	if (textSplitter) {
		if ("splitText" in textSplitter) textSplitterFunc = textSplitter.splitText.bind(textSplitter);
	...
	switch (messageType) {
		...
		case "tool":
			if ("tool_call_id" in fields) {
				if (returnChunk) chunk = new require_messages_tool.ToolMessageChunk(fields);
		...
		case "generic":
			if ("role" in fields) {
				if (returnChunk) chunk = new require_chat.ChatMessageChunk(fields);
