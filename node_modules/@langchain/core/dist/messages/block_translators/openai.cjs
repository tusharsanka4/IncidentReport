...
function mapReasoningItemToV1(item) {
	if (!require_utils._isArray(item.summary)) return void 0;
	return {
		type: "reasoning",
		reasoning: item.summary.reduce((acc, part) => {
			if (require_utils._isObject(part) && require_utils._isString(part.text)) return `${acc}${part.text}`;
			return acc;
		}, ""),
		...require_utils._isString(item.id) ? { id: item.id } : {},
		...require_utils._isString(item.encrypted_content) ? { encrypted_content: item.encrypted_content } : {}
	};
}
function* mapToolOutputToV1Blocks(toolOutput) {
	...
	if (require_utils._isContentBlock(toolOutput, "file_search_call")) {
		yield {
			id: toolOutput.id,
			type: "server_tool_call",
			name: "file_search",
	...
	if (require_utils._isContentBlock(toolOutput, "computer_call")) {
		yield {
			type: "non_standard",
			value: toolOutput
		};
	...
	if (require_utils._isContentBlock(toolOutput, "code_interpreter_call")) {
		...
		if (require_utils._isArray(toolOutput.outputs)) {
			...
			for (const output of toolOutput.outputs) if (require_utils._isContentBlock(output, "logs")) yield {
				type: "server_tool_call_result",
	...
	if (require_utils._isContentBlock(toolOutput, "mcp_call")) {
		yield {
			id: toolOutput.id,
			type: "server_tool_call",
			name: "mcp_call",
	...
	if (require_utils._isContentBlock(toolOutput, "mcp_list_tools")) {
		yield {
			id: toolOutput.id,
			type: "server_tool_call",
			name: "mcp_list_tools",
	...
	if (require_utils._isContentBlock(toolOutput, "mcp_approval_request")) {
		yield {
			type: "non_standard",
			value: toolOutput
		};
	...
	if (require_utils._isContentBlock(toolOutput, "tool_search_call")) {
	...
	if (require_utils._isContentBlock(toolOutput, "tool_search_output")) {
	...
	if (require_utils._isContentBlock(toolOutput, "image_generation_call")) {
...
function convertToV1FromResponses(message) {
	...
	const hasRawOutput = require_utils._isArray(rawOutput) && rawOutput.length > 0;
	...
	if (hasRawOutput) rawOutput.forEach((rawItem, index) => {
		if (!require_utils._isObject(rawItem) || !require_utils._isString(rawItem.type)) return;
		if (rawItem.type === "reasoning" && require_utils._isString(rawItem.id)) positionByKey.set(`reasoning:${rawItem.id}`, index);
		else if ((rawItem.type === "function_call" || rawItem.type === "custom_tool_call" || rawItem.type === "computer_call") && require_utils._isString(rawItem.call_id)) positionByKey.set(`tool_call:${rawItem.call_id}`, index);
		else if (rawItem.type === "message") {
			messageItemCount += 1;
			onlyMessageItemIndex = index;
		} else if (require_utils._isString(rawItem.id)) positionByKey.set(`other:${rawItem.type}:${rawItem.id}`, index);
	});
	...
	function* iterateContent() {
		if (hasRawOutput) {
			for (const rawItem of rawOutput) if (require_utils._isObject(rawItem) && rawItem.type === "reasoning") {
				const block = mapReasoningItemToV1(rawItem);
				if (block) yield [block, require_utils._isString(rawItem.id) ? `reasoning:${rawItem.id}` : void 0];
			}
		} else if (require_utils._isObject(message.additional_kwargs?.reasoning) && require_utils._isArray(message.additional_kwargs.reasoning.summary)) {
		...
		for (const block of content) if (require_utils._isContentBlock(block, "text")) {
			...
			const extras = require_utils._isObject(existingExtras) ? { ...existingExtras } : {};
			if (require_utils._isString(phase)) extras.phase = phase;
		...
		if (require_utils._isObject(message.additional_kwargs) && require_utils._isArray(message.additional_kwargs.tool_outputs)) for (const toolOutput of message.additional_kwargs.tool_outputs) {
			const key = require_utils._isObject(toolOutput) && require_utils._isString(toolOutput.type) && require_utils._isString(toolOutput.id) ? `other:${toolOutput.type}:${toolOutput.id}` : void 0;
...
exports.convertToV1FromChatCompletions = convertToV1FromChatCompletions;
exports.convertToV1FromChatCompletionsChunk = convertToV1FromChatCompletionsChunk;
exports.convertToV1FromChatCompletionsInput = convertToV1FromChatCompletionsInput;
exports.convertToV1FromResponses = convertToV1FromResponses;
exports.convertToV1FromResponsesChunk = convertToV1FromResponsesChunk;
