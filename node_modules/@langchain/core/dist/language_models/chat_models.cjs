...
const require_language_models_stream = require("./stream.cjs");
const require_language_models_compat = require("./compat.cjs");
...
var BaseChatModel = class BaseChatModel extends require_language_models_base.BaseLanguageModel {
	...
	*/
	async *_streamChatModelEvents(messages, options, runManager) {
		...
		yield* require_language_models_compat.convertChunksToEvents(chunks, { signal: options.signal });
	}
	streamEvents(input, options, streamOptions) {
		...
		return new require_language_models_stream.ChatModelStream(generator);
	}
	/**
	...
	*/
	...
	async *_streamIterator(input, options) {
		if (this._streamResponseChunks === BaseChatModel.prototype._streamResponseChunks || this.disableStreaming) yield this.invoke(input, options);
			...
			let llmOutput;
			if (generationChunk && require_ai.AIMessageChunk.isInstance(generationChunk.message) && generationChunk.message.usage_metadata !== void 0) llmOutput = { tokenUsage: {
				promptTokens: generationChunk.message.usage_metadata.input_tokens,
				completionTokens: generationChunk.message.usage_metadata.output_tokens,
	...
	*/
	...
	async _generateUncached(messages, parsedOptions, handledOptions, startedRunManagers) {
		...
		if (hasChatModelStreamEventHandler && !this.disableStreaming && baseMessages.length === 1 && (this._streamChatModelEvents !== BaseChatModel.prototype._streamChatModelEvents || this._streamResponseChunks !== BaseChatModel.prototype._streamResponseChunks)) try {
			...
			if (outputVersion === "v1") aggregated.message = require_utils$1.castStandardMessageContent(aggregated.message);
			generations.push([aggregated]);
			let llmOutput;
			if (require_ai.AIMessageChunk.isInstance(aggregated.message) && aggregated.message.usage_metadata !== void 0) llmOutput = { tokenUsage: {
	...
	*/
	...
	withStructuredOutput(outputSchema, config) {
		...
		return require_language_models_structured_output.assembleStructuredOutputPipeline(llm, outputParser, includeRaw, includeRaw ? "StructuredOutputRunnable" : "StructuredOutput");
	}
};
/**
