...
import { AIMessage, AIMessageChunk, isAIMessage } from "../messages/ai.js";
...
var BaseChatModel = class BaseChatModel extends BaseLanguageModel {
	...
	*/
	...
	async *_streamIterator(input, options) {
		if (this._streamResponseChunks === BaseChatModel.prototype._streamResponseChunks || this.disableStreaming) yield this.invoke(input, options);
			...
			let llmOutput;
			if (generationChunk && AIMessageChunk.isInstance(generationChunk.message) && generationChunk.message.usage_metadata !== void 0) llmOutput = { tokenUsage: {
	...
	*/
	...
	async _generateUncached(messages, parsedOptions, handledOptions, startedRunManagers) {
		...
		if (hasChatModelStreamEventHandler && !this.disableStreaming && baseMessages.length === 1 && (this._streamChatModelEvents !== BaseChatModel.prototype._streamChatModelEvents || this._streamResponseChunks !== BaseChatModel.prototype._streamResponseChunks)) try {
			...
			let llmOutput;
			if (AIMessageChunk.isInstance(aggregated.message) && aggregated.message.usage_metadata !== void 0) llmOutput = { tokenUsage: {
	...
	*/
	...
	withStructuredOutput(outputSchema, config) {
		...
		return assembleStructuredOutputPipeline(llm, outputParser, includeRaw, includeRaw ? "StructuredOutputRunnable" : "StructuredOutput");
	}
};
/**
