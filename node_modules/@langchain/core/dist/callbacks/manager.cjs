...
function coalesceTracers(handlers, inheritableHandlers) {
	...
	for (const handler of handlers) if (handler instanceof require_tracers_tracer_langchain.LangChainTracer) fold(handler);
	else coalescedHandlers.push(handler);
	for (const handler of inheritableHandlers) if (handler instanceof require_tracers_tracer_langchain.LangChainTracer && !groups.has(getTracerRunStoreKey(handler))) fold(handler);
	const seenTracerStores = /* @__PURE__ */ new Set();
	return {
		handlers: coalescedHandlers,
		inheritableHandlers: inheritableHandlers.flatMap((handler) => {
			if (!(handler instanceof require_tracers_tracer_langchain.LangChainTracer)) return [handler];
			const key = getTracerRunStoreKey(handler);
	...
	*/
	...
	async handleToolStart(tool, input, runId = require_utils_uuid_index.v7(), _parentRunId = void 0, _tags = void 0, _metadata = void 0, runName = void 0, toolCallId = void 0) {
		await Promise.all(this.handlers.map((handler) => {
			...
			return require_callbacks.consumeCallback(async () => {
				try {
					await handler.handleToolStart?.(tool, typeof input === "string" ? input : JSON.stringify(input), runId, this._parentRunId, this.tags, this.metadata, runName, toolCallId);
				} catch (err) {
