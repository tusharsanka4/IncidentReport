...
function coalesceTracers(handlers, inheritableHandlers) {
	...
	for (const handler of handlers) if (handler instanceof LangChainTracer) fold(handler);
	else coalescedHandlers.push(handler);
	for (const handler of inheritableHandlers) if (handler instanceof LangChainTracer && !groups.has(getTracerRunStoreKey(handler))) fold(handler);
