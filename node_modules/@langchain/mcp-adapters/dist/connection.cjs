...
var ConnectionManager = class {
	...
	async createClient(...args) {
		...
		if (this.#hooks.onInitialized) mcpClient.setNotificationHandler(_modelcontextprotocol_sdk_types_js.InitializedNotificationSchema, () => this.#hooks.onInitialized?.({
			server: serverName,
			options
		}));
		if (this.#hooks.onCancelled) mcpClient.setNotificationHandler(_modelcontextprotocol_sdk_types_js.CancelledNotificationSchema, (notification) => {
		...
		if (this.#hooks.onPromptsListChanged) mcpClient.setNotificationHandler(_modelcontextprotocol_sdk_types_js.PromptListChangedNotificationSchema, () => this.#hooks.onPromptsListChanged?.({
			server: serverName,
			options
		}));
		if (this.#hooks.onResourcesListChanged) mcpClient.setNotificationHandler(_modelcontextprotocol_sdk_types_js.ResourceListChangedNotificationSchema, () => this.#hooks.onResourcesListChanged?.({
			server: serverName,
			options
		}));
		if (this.#hooks.onResourcesUpdated) mcpClient.setNotificationHandler(_modelcontextprotocol_sdk_types_js.ResourceUpdatedNotificationSchema, (notification) => this.#hooks.onResourcesUpdated?.(notification.params, {
			server: serverName,
			options
		}));
		if (this.#hooks.onRootsListChanged) mcpClient.setNotificationHandler(_modelcontextprotocol_sdk_types_js.RootsListChangedNotificationSchema, () => this.#hooks.onRootsListChanged?.({
			server: serverName,
			options
		}));
		if (this.#hooks.onToolsListChanged) mcpClient.setNotificationHandler(_modelcontextprotocol_sdk_types_js.ToolListChangedNotificationSchema, () => this.#hooks.onToolsListChanged?.({
			server: serverName,
			options
		}));
	...
	*/
	#queryConnection(options) {
		...
		const [key, connection] = [...this.#connections.entries()].find(([key]) => {
			if (options.headers && options.authProvider) return key.serverName === options.serverName && key.headers === headers && key.authProvider === options.authProvider;
			if (options.headers && !options.authProvider) return key.serverName === options.serverName && key.headers === headers;
			if (options.authProvider && !options.headers) return key.serverName === options.serverName && key.authProvider === options.authProvider;
	...
	*/
	...
	getTransport(opts) {
		...
		*/
		if ("listTools" in opts) return [...this.#connections.values()].find((connection) => connection.client === opts)?.transport;
	...
	async #createStreamableHTTPTransport(serverName, args) {
		...
		return Object.keys(options).length > 0 ? new _modelcontextprotocol_sdk_client_streamableHttp_js.StreamableHTTPClientTransport(new URL(url), options) : new _modelcontextprotocol_sdk_client_streamableHttp_js.StreamableHTTPClientTransport(new URL(url));
	}
	/**
	...
	*/
	async #createSSETransport(serverName, args) {
		...
		if (headers) {
			options.eventSourceInit = { fetch: async (url, init) => {
		...
		return new _modelcontextprotocol_sdk_client_sse_js.SSEClientTransport(new URL(url), options);
	}
	#createStdioTransport(options) {
		const { command, args, env, stderr, cwd } = options;
		return new _modelcontextprotocol_sdk_client_stdio_js.StdioClientTransport({
			command,
			args,
			stderr,
			cwd,
