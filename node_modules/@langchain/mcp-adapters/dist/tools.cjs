...
async function _toolOutputToContentBlocks(content, useStandardContentBlocks, client, toolName, serverName) {
	...
	switch (content.type) {
		...
		default: throw new ToolException(`MCP tool '${toolName}' on server '${serverName}' returned a content block with unexpected type "${content.type}." Expected one of ${require_types.callToolResultContentTypes.map((t) => `"${t}"`).join(", ")}.`);
	}
}
...
function _getOutputTypeForContentType(contentType, outputHandling) {
	...
	return require_types._resolveDetailedOutputHandling(outputHandling)[contentType] ?? (contentType === "resource" ? "artifact" : "content");
}
/**
...
async function _callTool({ serverName, toolName, client, args, config, useStandardContentBlocks, outputHandling, onProgress, beforeToolCall, afterToolCall }) {
	try {
		...
		if (_langchain_core_messages.ToolMessage.isInstance(interceptedResult.result)) return [interceptedResult.result.contentBlocks, []];
		if (interceptedResult?.result instanceof _langchain_langgraph.Command) return interceptedResult.result;
...
async function loadMcpTools(serverName, client, options) {
	...
	return (await Promise.all(mcpTools.filter((tool) => !!tool.name).map(async (tool) => {
		try {
			...
			const dst = new _langchain_core_tools.DynamicStructuredTool({
