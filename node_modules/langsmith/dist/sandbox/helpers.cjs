...
exports.throwIfNotReady = throwIfNotReady;
exports.handleClientHttpError = handleClientHttpError;
exports.handleSandboxHttpError = handleSandboxHttpError;
exports.assertRunConfigNotCombined = assertRunConfigNotCombined;
exports.resolveCloseInput = resolveCloseInput;
exports.buildRangeHeader = buildRangeHeader;
exports.fileStatFromResponse = fileStatFromResponse;
exports.fileChunkFromResponse = fileChunkFromResponse;
exports.handleFileHttpError = handleFileHttpError;
    ...
    try {
        ...
        if (detail && typeof detail === "object" && !Array.isArray(detail)) {
            ...
            return {
                ...
                message: typeof detail.error_id === "string" && detail.error_id
 ...
 */
async function throwIfNotReady(response, name) {
    ...
    throw new errors_js_1.LangSmithSandboxNotReadyError(data.message || `Sandbox '${name}' is not ready`);
}
/**
 ...
 * replacement.
 */
function assertRunConfigNotCombined(options) {
    if (options.runConfig !== undefined &&
        (options.env !== undefined || options.cwd !== undefined)) {
        throw new errors_js_1.LangSmithValidationError("Cannot combine runConfig with the deprecated env/cwd options. " +
 ...
 */
function resolveCloseInput(closeInput, pty) {
    if (pty)
        return false;
...
function buildRangeHeader(options) {
    const { start, end, suffixBytes } = options;
    if (suffixBytes !== undefined) {
        if (start !== undefined || end !== undefined) {
            throw new errors_js_1.LangSmithValidationError("Cannot combine suffixBytes with start/end.");
        }
        if (suffixBytes <= 0) {
            throw new errors_js_1.LangSmithValidationError("suffixBytes must be positive.");
        }
        return `bytes=-${suffixBytes}`;
    }
    if (start === undefined) {
        throw new errors_js_1.LangSmithValidationError("Provide start (with optional end), or suffixBytes.");
    }
    if (start < 0) {
        throw new errors_js_1.LangSmithValidationError("start must not be negative.");
    }
    if (end === undefined) {
        return `bytes=${start}-`;
    }
    if (end < start) {
        throw new errors_js_1.LangSmithValidationError("end must not precede start.");
    }
...
function fileStatFromResponse(response) {
...
async function fileChunkFromResponse(response) {
...
async function handleFileHttpError(response, path, sandboxName) {
    if (response.status === 404) {
        throw new errors_js_1.LangSmithResourceNotFoundError(`File '${path}' not found in sandbox '${sandboxName}'`, "file");
    }
    if (response.status === 416) {
        throw new errors_js_1.LangSmithSandboxOperationError(`Requested range for '${path}' starts past the end of the file ` +
            `(${response.headers.get("content-range") ?? "unknown size"}).`, "read");
    }
