...
class WSStreamControl {
    constructor() {
        ...
        Object.defineProperty(this, "_closeStdinPending", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
    ...
    return new Promise((resolve, reject) => {
        ...
        ws.on("unexpected-response", (req, res) => {
            ...
            if (RETRYABLE_HANDSHAKE_STATUS_CODES.has(res.statusCode ?? 0)) {
                reject(new errors_js_1.LangSmithSandboxRetryableConnectionError(message, retryAfterSeconds(res.headers)));
            }
            else {
                reject(new errors_js_1.LangSmithSandboxConnectionError(message));
            }
        });
...
async function runWsStream(dataplaneUrl, apiKey, command, options = {}) {
    const { timeout = 60, env, cwd, shell = "/bin/bash", onStdout, onStderr, commandId, idleTimeout = 300, killOnDisconnect = false, ttlSeconds = 600, pty, runConfig, closeStdin, headers: extraHeaders, openTimeout = exports.WS_OPEN_TIMEOUT, } = options;
