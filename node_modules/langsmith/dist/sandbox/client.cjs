...
const access_delegation_js_1 = require("./access_delegation.cjs");
const service_url_js_1 = require("./service_url.cjs");
...
class SandboxClient {
     ...
     */
    async serviceUrl(name, options) {
        const { port, expiresInSeconds, access, signal } = options;
        if (!Number.isInteger(port) || port < 1 || port > 65535) {
            throw new errors_js_1.LangSmithValidationError(`port must be between 1 and 65535 (got ${port})`, "port");
        }
        if (access !== undefined &&
            access !== "restricted" &&
            access !== "workspace") {
            throw new errors_js_1.LangSmithValidationError(`access must be "restricted" or "workspace" (got ${String(access)})`, "access");
        }
        const loginMode = access !== undefined;
        if (expiresInSeconds !== undefined && loginMode) {
            throw new errors_js_1.LangSmithValidationError(`expiresInSeconds does not apply to access "${access}": a LangSmith ` +
                "login URL carries no token and does not expire", "expiresInSeconds");
        }
        ...
        if (loginMode) {
            return new service_url_js_1.ServiceLoginUrl(data);
        }
        ...
        return new service_url_js_1.ServiceUrl(data, () => this.serviceUrl(name, {
            port,
            expiresInSeconds,
        }));
    }
    ...
    async createSandbox(snapshotIdOrOptions, options = {}) {
        ...
        const resolvedOptions = typeof snapshotIdOrOptions === "object" && snapshotIdOrOptions !== null
            ? snapshotIdOrOptions
            : options;
        const { snapshotName, name, timeout = 30, waitForReady = true, idleTtlSeconds, deleteAfterStopSeconds, vCpus, memBytes, fsCapacityBytes, mountConfig, proxyConfig, runConfig, accessDelegation, } = resolvedOptions;
        ...
        if (accessDelegation !== undefined) {
            payload.access_delegation = (0, access_delegation_js_1.validateAccessDelegation)(accessDelegation);
        }
    ...
    async updateSandbox(name, newNameOrOptions) {
        const options = typeof newNameOrOptions === "string"
            ...
            : newNameOrOptions;
        const { newName, idleTtlSeconds, deleteAfterStopSeconds, proxyConfig, runConfig, } = options;
        (0, helpers_js_1.validateTtl)(idleTtlSeconds, "idleTtlSeconds");
        (0, helpers_js_1.validateTtl)(deleteAfterStopSeconds, "deleteAfterStopSeconds");
        ...
        if (!response.ok) {
            ...
            if (proxyConfig !== undefined) {
                await (0, helpers_js_1.throwIfNotReady)(response, name);
            }
            await (0, helpers_js_1.handleClientHttpError)(response);
        }
        const data = (await response.json());
    ...
    async captureSnapshot(sandboxName, name, options = {}) {
        const { dockerImage, fsCapacityBytes, runConfig, timeout = 60, signal, } = options;
