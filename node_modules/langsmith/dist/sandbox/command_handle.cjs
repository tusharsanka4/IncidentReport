 ...
 */
function acknowledgesReconnect(msg, commandId) {
    const acked = msg.command_id;
    if (acked === commandId)
        return true;
    console.warn(`Ignoring reconnect acknowledgement for command ${String(acked)} while attached to ${String(commandId)}`);
    return false;
}
/**
...
class CommandHandle {
    ...
    constructor(messageStream, control, sandbox, options) {
        ...
        Object.defineProperty(this, "_stdinClosed", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "_pty", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        while (true) {
            try {
                ...
                if (this._reconnectAttempts > CommandHandle.MAX_AUTO_RECONNECTS) {
                    throw new errors_js_1.LangSmithSandboxConnectionError(`Failed to reattach to the command ${this._reconnectAttempts} times in succession, giving up`);
                }
                const isHotReload = e instanceof errors_js_1.LangSmithSandboxServerReloadError;
                if (!isHotReload) {
                    const delay = Math.min(CommandHandle.BACKOFF_BASE * 2 ** (this._reconnectAttempts - 1), CommandHandle.BACKOFF_MAX);
    ...
    sendInput(data) {
        if (this._stdinClosed) {
            throw new errors_js_1.LangSmithSandboxOperationError("stdin is closed for this command. Non-PTY commands close stdin by " +
                "default so a command that reads it sees EOF instead of hanging; " +
                "pass closeInput: false to run() to stream input into it.", "sendInput");
        }
        if (this._control) {
     ...
     */
    closeInput() {
        if (this._pty || this._stdinClosed)
            return;
