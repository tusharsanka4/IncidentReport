...
import { ServiceLoginUrl, ServiceUrl, } from "./service_url.js";
...
import { handleClientHttpError, handleSandboxCreationError, throwIfNotReady, validateTtl, } from "./helpers.js";
...
export class SandboxClient {
    ...
    async serviceUrl(name, options) {
        ...
        if (!Number.isInteger(port) || port < 1 || port > 65535) {
            throw new LangSmithValidationError(`port must be between 1 and 65535 (got ${port})`, "port");
        }
        if (access !== undefined &&
            access !== "restricted" &&
            access !== "workspace") {
            throw new LangSmithValidationError(`access must be "restricted" or "workspace" (got ${String(access)})`, "access");
        }
        const loginMode = access !== undefined;
        if (expiresInSeconds !== undefined && loginMode) {
            throw new LangSmithValidationError(`expiresInSeconds does not apply to access "${access}": a LangSmith ` +
        ...
        return new ServiceUrl(data, () => this.serviceUrl(name, {
            port,
            expiresInSeconds,
        }));
    }
