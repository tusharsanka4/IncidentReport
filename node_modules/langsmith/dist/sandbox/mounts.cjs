...
exports.contextHubMount = contextHubMount;
 ...
 */
function contextHubMount({ id, mountPath, repo, initialPullOnly, }) {
    const mount = {
        id: requireNonEmptyString(id, "id"),
        type: "contexthub",
        mount_path: requireNonEmptyString(mountPath, "mountPath"),
...
function normalizeMounts(mounts) {
    ...
    return mounts.map((mount) => {
        ...
        if (mount.type !== "s3" &&
            ...
            throw new Error("mountConfig only supports s3, gcs, git, and contexthub mounts");
        }
...
function normalizeMountAuth(auth) {
    ...
    for (const block of auth) {
        ...
        if (provider === "aws") {
            ...
            const roleArn = (0, proxy_config_js_1.getAwsRoleArn)(aws);
 ...
 */
function mountConfig({ auth = [], mounts, proxyConfig, }) {
    ...
    const proxyProviders = new Set((proxyConfig?.rules ?? [])
        .filter((rule) => rule?.enabled === true)
        .map((rule) => rule.type));
    if (mountProviders.has("s3") &&
