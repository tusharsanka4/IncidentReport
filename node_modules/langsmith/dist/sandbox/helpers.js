 ...
 */
export async function throwIfNotReady(response, name) {
    ...
    throw new LangSmithSandboxNotReadyError(data.message || `Sandbox '${name}' is not ready`);
}
/**
...
export function assertRunConfigNotCombined(options) {
    if (options.runConfig !== undefined &&
        ...
        throw new LangSmithValidationError("Cannot combine runConfig with the deprecated env/cwd options. " +
 ...
 */
export function resolveCloseInput(closeInput, pty) {
    if (pty)
        return false;
...
export function buildRangeHeader(options) {
    const { start, end, suffixBytes } = options;
    if (suffixBytes !== undefined) {
        if (start !== undefined || end !== undefined) {
            throw new LangSmithValidationError("Cannot combine suffixBytes with start/end.");
        }
        if (suffixBytes <= 0) {
    ...
    if (start === undefined) {
        throw new LangSmithValidationError("Provide start (with optional end), or suffixBytes.");
    }
    if (start < 0) {
...
export function fileStatFromResponse(response) {
...
export async function fileChunkFromResponse(response) {
...
export async function handleFileHttpError(response, path, sandboxName) {
    if (response.status === 404) {
        throw new LangSmithResourceNotFoundError(`File '${path}' not found in sandbox '${sandboxName}'`, "file");
    }
    if (response.status === 416) {
        throw new LangSmithSandboxOperationError(`Requested range for '${path}' starts past the end of the file ` +
