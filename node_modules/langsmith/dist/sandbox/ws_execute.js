    ...
    return new Promise((resolve, reject) => {
        ...
        ws.on("unexpected-response", (req, res) => {
            ...
            if (RETRYABLE_HANDSHAKE_STATUS_CODES.has(res.statusCode ?? 0)) {
                reject(new LangSmithSandboxRetryableConnectionError(message, retryAfterSeconds(res.headers)));
            }
            else {
...
export async function runWsStream(dataplaneUrl, apiKey, command, options = {}) {
    const { timeout = 60, env, cwd, shell = "/bin/bash", onStdout, onStderr, commandId, idleTimeout = 300, killOnDisconnect = false, ttlSeconds = 600, pty, runConfig, closeStdin, headers: extraHeaders, openTimeout = WS_OPEN_TIMEOUT, } = options;
